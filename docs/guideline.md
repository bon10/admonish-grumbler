...
AIによる投稿分析結果。週次・月次で生成され、ストレススコア・幸福度・感情スコア・トピック抽出・カウンセリングメッセージなどを含む。ユーザーが残したフィードバックを保持する。

### FeedbackProfile（フィードバックプロファイル）

ユーザーがサマリーに残したフィードバックを、以降すべての分析へ恒久的に反映するためのプロファイル。アプリ全体で1つだけ存在する。

生のフィードバックをそのまま溜めると分析プロンプトが件数分だけ膨張するため、AIが「指示（FeedbackInstruction）」の集合へ正規化して保持する。同じ趣旨の指示は1つにまとめて言及回数（weight）を加算し、矛盾する要求は新しいもので置き換える。件数と総文字数に上限を設け、超過分は言及回数が少なく最終言及の古い指示から落とす。

各指示は適用範囲（scope）として全体・週次・月次のいずれかを持ち、恒久的な要求か1回きりの感想かを persistent で区別する。1回きりの指示は適用先の分析に使われた時点で破棄される。
